...
from contextlib import contextmanager       #  将生成器函数转换为上下文管理
...
# Proxy 类：延迟初始化，用于模型绑定数据库
class Proxy(object):
    """
    Create a proxy or placeholder for another object.
    """
    __slots__ = ('obj', '_callbacks')# 用tuple定义允许绑定的属性名称
...
def __bind_database__(meth):
    @wraps(meth)   # @wraps(func) 是一个装饰器，用于帮助保留被装饰函数的元数据。
    def inner(self, *args, **kwargs):
