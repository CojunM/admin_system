...
class Notification(BaseModel):
    ...
    id =  IntegerField(primary_key=True, comment="通知ID")
    title = StringField(max_length=128, nullable=False, comment="通知标题")
    content = StringField(nullable=False, comment="通知内容")
    type =  IntegerField(default=1, comment="1-系统通知 2-业务通知")
    user_id = ForeignKeyField(to=User, nullable=True, comment="指定用户（NULL为全体）")
    is_read =  IntegerField(default=0, comment="是否已读 0-未读 1-已读")
